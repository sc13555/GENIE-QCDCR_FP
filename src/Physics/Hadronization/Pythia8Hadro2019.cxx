...
void Pythia8Hadro2019::Configure(const Registry & config)
{
  ...
  this->LoadConfig();
  Pythia8::Pythia* gPythia = Pythia8Singleton::Instance()->Pythia8();
  std::string cmnd_file;

  bool found = GetParam("Pythia8ConfigFile", cmnd_file);
  if (found && !cmnd_file.empty()) {
    LOG("PythiaHad", pINFO) << "Reading config file: " << cmnd_file;

    std::ifstream f(cmnd_file.c_str());
    if (!f.good()) {
      LOG("PythiaHad", pFATAL) << "File not found: " << cmnd_file;
      exit(1);
    }

    gPythia->readFile(cmnd_file);
    gPythia->init();
  } else {
    LOG("PythiaHad", pWARN) << "No Pythia8ConfigFile specified! Using fallback.";
    gPythia->readString("HadronLevel:all = on");
    gPythia->init();
  }
}
//____________________________________________________________________________
